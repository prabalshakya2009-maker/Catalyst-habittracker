document.addEventListener("DOMContentLoaded", () => {
    ...
    // --- V11 AUDIO ENGINE ---
    // Using open source audio from Google's standard UI sounds to prevent CORS blocking
    ...
    const defaultState = {
        ...
        weeklyXP: 0, monthlyXP: 0, history: [],
        config: {
            exam: 'JEE', metric: 'AIR',
    ...
    let userData = JSON.parse(localStorage.getItem('jeeNexusData_v11'));
    
    // V11 Migration
    if (!userData) {
        let oldData = JSON.parse(localStorage.getItem('jeeNexusData_v10')) || JSON.parse(localStorage.getItem('jeeNexusData_v9'));
        if (oldData) {
            userData = oldData;
            localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData));
        } else {
    ...
    function renderDirectives() {
        ['morning', 'afternoon', 'evening'].forEach(block => {
            ...
            userData.directives[block].forEach(task => {
                ...
                row.innerHTML = `
                    <label class="custom-checkbox">
                        ...
                        <span>${task.text} <span style="color:var(--primary-neon); font-size:0.7rem;">(+${task.xp}XP)</span></span>
                    </label>
                ...
                row.addEventListener('dragend', function() {
                    ...
                    // Save new array order after drop
                    const newArray = [];
                    container.querySelectorAll('.directive-row').forEach(r => {
                        const rId = parseInt(r.getAttribute('data-id'));
                        const rTask = userData.directives[block].find(t => t.id === rId);
            ...
            container.addEventListener('dragover', e => {
                e.preventDefault();
                if(dragSourceBlock !== block) return; // Prevent dragging between morning/evening
        ...
        document.querySelectorAll('.directive-checkbox').forEach(box => {
            box.addEventListener('change', (e) => {
                ...
                if (task) {
                    ...
                    if (task.completed) {
                        addGlobalXP(task.xp);
                        sfxCheck.play().catch(e => console.log("Audio play prevented by browser")); // V11 Audio
                    } else {
                        addGlobalXP(-task.xp);
                    }
    ...
    document.querySelectorAll('.task-add-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            textInput.value = ''; renderDirectives(); localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData)); updateUI();
        });
    });

    // --- V11 DATA EXPORT / IMPORT ---
    if(document.getElementById('exportDataBtn')) {
        document.getElementById('exportDataBtn').addEventListener('click', () => {
            ...
            downloadAnchorNode.setAttribute("download", "nexus_os_backup.json");
        ...
        document.getElementById('importDataFile').addEventListener('change', (e) => {
            ...
            reader.onload = (event) => {
                try {
                    ...
                    if(importedData.config && importedData.directives) {
                        localStorage.setItem('jeeNexusData_v11', JSON.stringify(importedData));
                        alert("Profile Restore Successful! The dashboard will now reload.");
                        location.reload();
    ...
    function applyConfig() {
        ...
        if(window.radarChartInstance) { window.radarChartInstance.data.labels = [c.sub1, c.sub2, c.sub3, c.sub4, c.sub5]; window.radarChartInstance.update(); }
    ...
    if(document.getElementById('saveConfigBtn')) {
        document.getElementById('saveConfigBtn').addEventListener('click', () => {
            ...
            localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData)); applyConfig(); alert("Settings applied globally!");
        });
    }

    // --- V11 ACHIEVEMENT BADGE RENDERER ---
    function updateBadges() {
        ...
        const badges = [
            ...
            { id: 'b5', name: "Grinder", icon: "fa-dumbbell", unlocked: userData.tasksCompletedToday >= 10 }
        ];

        container.innerHTML = '';
        badges.forEach(b => {
            const div = document.createElement('div');
            div.className = `badge-item ${b.unlocked ? 'badge-unlocked' : 'badge-locked'}`;
            div.innerHTML = `<div class="badge-icon-box"><i class="fa-solid ${b.icon}"></i></div><span class="badge-title">${b.name}</span>`;
    ...
    function updateUI() {
        ...
        if(userData.history && userData.history.length > 0) {
            avgDiscipline = Math.round(userData.history.reduce((a, b) => a + b.discipline, 0) / userData.history.length);
        }
        ...
        updateBadges(); // Check unlocked achievements
        localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData));
    }
    ...
    function updateClass11Progress() {
        ...
        chapCount11.innerText = `${userData.class11Completed} / 40 Ch`; progBar11.style.width = `${percentage}%`; progBar11.innerText = `${percentage}%`;
    }

    if(document.getElementById('addChap11')) document.getElementById('addChap11').addEventListener('click', () => { if (userData.class11Completed < 40) { userData.class11Completed++; updateClass11Progress(); localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData)); } });
    if(document.getElementById('subChap11')) document.getElementById('subChap11').addEventListener('click', () => { if (userData.class11Completed > 0) { userData.class11Completed--; updateClass11Progress(); localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData));} });
        ...
        if (!isRunning) {
            ...
            timerInterval = setInterval(() => {
                if (timeLeft > 0) { timeLeft--; updateTimerDisplay(); } 
                    ...
                    localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData));
                    renderFocusLog(); 
                    sfxAlarm.play().catch(e=>console.log("Audio play prevented")); // V11 Alarm Fire
    ...
    function renderErrorLog() {
        ...
        document.querySelectorAll('.delete-err-btn').forEach(btn => {
            btn.addEventListener('click', (e) => { userData.errorLogs.splice(parseInt(e.currentTarget.getAttribute('data-index')), 1); localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData)); renderErrorLog(); });
        });
    }
    ...
    if(document.getElementById('addErrorBtn')) document.getElementById('addErrorBtn').addEventListener('click', () => {
        ...
        addGlobalXP(50); localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData));
    ...
    function renderBacklogs() {
        ...
        document.querySelectorAll('.claim-bounty-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData)); renderBacklogs(); updateUI();
            });
        });
    }
    ...
    if(document.getElementById('addBacklogBtn')) document.getElementById('addBacklogBtn').addEventListener('click', () => {
        ...
        localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData)); backlogInput.value = ''; renderBacklogs();
    });
    renderBacklogs();
    ...
    document.querySelectorAll('.pyq-add').forEach(btn => { btn.addEventListener('click', (e) => { const sub = e.currentTarget.getAttribute('data-subject'); if (userData.pyqCounts[sub] < 300) { userData.pyqCounts[sub] += 5; localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData)); updatePYQ(); } }); });
    document.querySelectorAll('.pyq-sub').forEach(btn => { btn.addEventListener('click', (e) => { const sub = e.currentTarget.getAttribute('data-subject'); if (userData.pyqCounts[sub] > 0) { userData.pyqCounts[sub] = Math.max(0, userData.pyqCounts[sub] - 5); localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData)); updatePYQ(); } }); });
    updatePYQ();
    ...
    if (ctxRadar) {
        window.radarChartInstance = new Chart(ctxRadar.getContext('2d'), { type: 'radar', data: { labels: [userData.config.sub1, userData.config.sub2, userData.config.sub3, userData.config.sub4, userData.config.sub5], datasets: [{ label: 'Mastery', data: userData.subjectMastery, backgroundColor: 'rgba(0, 242, 254, 0.2)', borderColor: '#00f2fe', pointBackgroundColor: '#00f2fe', borderWidth: 2 }] }, options: { scales: { r: { ticks: { display: false, min: 0, max: 100 } } }, plugins: { legend: { display: false } } } });
        for(let i=0; i<5; i++) { const slider = document.getElementById(`mastery${i}`); if(slider) { slider.value = userData.subjectMastery[i]; slider.addEventListener('input', (e) => { userData.subjectMastery[i] = parseInt(e.target.value); localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData)); window.radarChartInstance.update(); }); } }
    }
    ...
    if (ctxBar) {
        ...
        for(let i=0; i<7; i++) { const input = document.getElementById(`hrs${i}`); if(input) { input.value = userData.weeklyHours[i]; input.addEventListener('input', (e) => { userData.weeklyHours[i] = parseFloat(e.target.value) || 0; localStorage.setItem('jeeNexusData_v11', JSON.stringify(userData)); barChart.update(); }); } }
    }
